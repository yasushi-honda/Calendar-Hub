...
3. **ダッシュボード作成** ⏳ 実装中（PR #3、Issue #81 c。PR #155 後続）: SLI 4 種 (API 5xx rate / API latency / Sync failures / Web 5xx rate) + 関連メトリクス (Sync gap / RRULE skip / TT session expired per accountId / Mail send fail) を 1 mosaic dashboard に集約 (`infra/setup-dashboard.sh` + `infra/dashboards/calendar-hub-slo.yaml`、計 7 widgets)
