...
- Status: Accepted（設計 + 一部実装中）
- Date: 2026-05-03（更新: 2026-06-26、実装フェーズ着手）
- Issue: #81
...
1. **ログバケット作成 + sink 設定** ✅ 実装済み（PR #154、2026-06-26）: `sync-logs`（90 日）/ `auth-logs`（180 日）バケット + 同名 sink を `infra/setup-log-retention.sh` で idempotent 構築
2. **SLI / SLO の Cloud Monitoring 設定** ⏳ 実装中（PR #155、Issue #81 b）:
   - `infra/setup-slo.sh` + `infra/slo-services/*.json` + `infra/slos/*.json` で 4 SLO 構築
   - gcloud CLI に `monitoring services / slos` 非対応のため REST API + curl 経由（jq でレスポンス parse）
   - **Sync SLO の Pragmatic 実装**: 総 sync 数を表すメトリクス未整備のため `windowsBased.metricSumInRange` で 1h 単位 failure 0 件を good window と定義。「99%/day」を「24h 中 99% の hour で failure 0 件」として近似。将来的に `calendar_hub_sync_attempt` 等の総数メトリクス追加後に true 成功率 SLO へ置換予定
3. **ダッシュボード作成** ⏳ 実装中（PR #156、Issue #81 c。PR #155 後続）: SLI 4 種 (API/Web requests by response class / API latency p50-95-99 / Sync failures) + 関連メトリクス (Sync gap / RRULE skip / TT session expired per accountId / Mail send fail) を 1 mosaic dashboard に集約 (`infra/setup-dashboard.sh` + `infra/dashboards/calendar-hub-slo.yaml`、計 7 widgets)
