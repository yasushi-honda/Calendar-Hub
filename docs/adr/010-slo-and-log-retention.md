...
99.5% は `min-instances=0` の cold start 起動失敗を見越した現実的水準。`min-instances` を上げる場合は SLO 99.9% への引き上げを検討する。
...
| API p95 レイテンシ | `run.googleapis.com/request_latencies` の `99_percentile`（`server_time` filter）             |
...
- 例外的に PII を残す場合は、Cloud Logging の `data masking`（regex masking policy）を `auth-logs` バケットに適用
...
- ADR-009（TimeTree session management）— ログプロトコルの先例
